...
define(['jquery', 'dust'], function ($, dust) {
	...
	DustRenderer = function (nougat) {
		...
		// Falcon change start
		// we had to change where the renderer was looking for the template
		// base url because of how sparta had it stored in the json response
		var DEFAULT_PATH = '/templates';
		dust.onLoad = function (name, callback) {
			var path = nougat.getContext().sys.links.templateBaseUrl || DEFAULT_PATH,
				template = path + "/" + name + '.js';
			// Falcon change end
